...
def test_methods():
    ...
    ds1.z = ds1.w
    assert (
        ds1.provenance[-1].__repr__()
        == "<Transformation(Array z has been replaced by w)>"
    )
    ds1.w = np.array([6, 7, 8])
    assert (
        ds1.provenance[-1].__repr__()
        == "<Transformation(Array w has been replaced by array([6, 7, 8]))>"
    )
...
def test_selections():
    ...
    assert ds2.provenance[-1].detail == (
        "Selection, x > 1, applied (#events before = 3, "
