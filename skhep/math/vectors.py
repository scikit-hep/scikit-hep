...
class Vector3D(object):
    ...
    def __iadd__(self, other):
        ...
        if not isinstance(other, Vector3D):
            raise InvalidOperationError(
                "invalid operation '+=' between a 'Vector3D' and a '{0}'".format(
    ...
    def __eq__(self, other):
        ...
        ##
        return (
            isequal(self[0], other[0])
            and isequal(self[1], other[1])
            and isequal(self[2], other[2])
        )
    ...
    def rotate(self, angle, *args):
        ...
        if len(args) == 1 and isinstance(args[0], Vector3D):
            ...
            raise TypeError(
                "Input object not a Vector3D nor an iterable with 3 elements."
            )
...
class LorentzVector(object):
    ...
    def transversemass(self):
        ...
        return sqrt(mt2) if mt2 >= 0.0 else -sqrt(-mt2)
    ...
    def mag(self):
        ...
        mag2 = self.mag2
        return sqrt(mag2) if mag2 >= 0.0 else -sqrt(-mag2)
    ...
    def __iadd__(self, other):
        ...
        if not isinstance(other, LorentzVector):
            raise InvalidOperationError(
                "invalid operation '+=' between a 'LorentzVector' and a '{0}'".format(
                    other.__class__.__name__
                )
            )
    ...
    def isspacelike(self):
        ...
        return self.mag2 < 0.0 and not isequal(self.mag2, 0.0)
